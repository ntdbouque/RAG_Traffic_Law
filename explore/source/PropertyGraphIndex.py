...
'''
Step in `_insert_node` function:
    1. Kiểm tra xem có nào không, nếu không thì trả về rỗng
    2. Trích xuất triplet từ nodes đầu vào (sử dụng SimpleLLMPathExtractor)
    -> Biến nội dung văn bản từ nodes thành cấu trúc graph (entites, relations)
    3. Xác nhận metadata có triplet 
    -> đảm bảo extract thành công
    4. Duyệt node đã extract lấy ra entites và relations
    -> Chuẩn bị dữ liệu để ingest vào graph
    5. Lọc ra những trùng lặp (áp dụng cho llama nodes-chunk và kg nodes-entity)
    6. Embedding node áp dụng cho cả llama nodes và kg nodes (EMBEDDING_MODE = EMBED)
'''