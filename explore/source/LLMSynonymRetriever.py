...
DEFAULT_SYNONYM_EXPAND_TEMPLATE = (
    ...
    "Must include original keywords\n"
    "----\n"
    "QUERY: {query_str}\n"
    "----\n"
    "KEYWORDS: "
...
class LLMSynonymRetriever(BasePGRetriever):
    ...
    async def aretrieve_from_graph(
        ...
        '''
        Steps:
            1. Generate synonym from LLM
            2. Parse output (ensure LLM output is normalized and reuse for Graph)
        ...
        matches = self._parse_llm_output(response)
        ic(matches)
        results = await self._aprepare_matches(matches, limit=limit or self._limit)
        ic(results)
        return results
