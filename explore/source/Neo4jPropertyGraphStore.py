...
class Neo4jPropertyGraphStore(PropertyGraphStore):
    ...
    def get(
        ...
        '''
        Steps:
            1. Construct Cypher query match __Node__
            2. Construct Cypher only match with ids
            3. Construct Cypher (properties)
            4. Get reality type (e.g. Person/ Organization/Location) and get all properties of nodes (except embedding and <id>)
            5. Run completed Cypher query in Neo4j
            6. Parse response from Neo4j
            7. Return nodes

        Notes:
            - IDS IS A STRING (e.g. Biển cảnh báo, Nồng độ cồn), SO THIS FUNCTION IS SO BAD!!!!
        '''
        cypher_statement = f"MATCH (e)"

        params = {}
        cypher_statement += "WHERE e.name IS NOT NULL "

        if names:
            cypher_statement += "AND e.name in $names "
            params["names"] = names
    ...
    def get_rel_map(
        ...
        Arguments:
            graph_nodes: List of starting points 
            depth: max depth of the path
            limit: max number of triplets 
            ignore_rels: list of relationship types to ignore
        Returns:
            List of triplets, each triplet is a list of [source_node, relation, target_node]

        """

        '''
        Steps:
            1. Get ids from graph_nodes
            2. Construct Cypher Query and run it
            3. Process ignore_rels
            4. Parse response to triplets
            5. Return triplets
        '''
        triples = []
    ...
    def vector_query(
        ...
        if not query.filters and self._supports_vector_index:
            ...
            ic('FILTERS OR NO SUPPORT VECTOR INDEX')
            data = self.structured_query(
